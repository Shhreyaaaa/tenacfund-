...
// Fetch Secured Loans data from `subsubcategory` table
$Moneymarket = $db->fetchAssoc($db->fireQuery("SELECT * FROM `subsubcategory` WHERE `subsubcategory`='Money Market' ORDER BY id DESC LIMIT 1"));
$productName = $Moneymarket[0]['subsubcategory'];
?>
<style>
.enquiry-box {
  max-width: 70%; 
...
<?php $moneyMarket=$db->fetchAssoc($db->fireQuery("select * from `subsubcategory` where `subsubcategory`='Money Market' order by id desc"));?>
   <!-- ======= Money Market Section ======= -->
    ...
    </section><!-- End Money Market Section -->
